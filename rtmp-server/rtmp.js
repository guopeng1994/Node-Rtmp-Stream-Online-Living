...
(function() {
  ...
  //数据库相关配置
  var sql = require('../server/dao/sql/SqlStatement.js')//常用sql语句
  var mysql = require('mysql');
  var conf = require('../server/conf/DBconf.js');
  ...
  RTMPSession = (function() {
    ...
    RTMPSession.prototype.respondConnect = function(commandMessage, callback) {
      ...
      if ((app !== config.liveApplicationName) && (app !== config.recordedApplicationName)) {
        logger.warn("[rtmp:client=" + this.clientid + "] 请求失败！不存在的APP名称: " + app);
    ...
    //推流端主动断开推流 当前可获取的是推流md5字符串
    //先根据字符串找到推流的主播用户，再修改用户的直播状态及新的推流md5
    pool.getConnection(function(err,connection){
      connection.query(sql.anchor.selectMd5CreateInfoByMd5,[streamName],function(error,result){
        connection.release();
        if(error){
          console.log(error);
          return;
        ...
        pool.getConnection(function(err,connection){
          ...
          let status = 0;//停止推流，直播状态置0
          connection.query(sql.anchor.startShow,[status,encoded,result[0].anch_name],function(error,result){
            connection.release();
            if(error){
              console.log(error);
              return;
  ...
  pool.getConnection((err,connection)=>{
      ...
      connection.query(sql.anchor.pushValidate,[urlInfo.pathname],(error,result)=>{
        connection.release();
        if(error){
          console.log(error);
          return;
        ...
        if(result.length == 0)//没有改数据，说明推流md5密钥不对
        {
          let session, sessionID;
          console.log("当前推流地址不正确!");
          if (sessions[this.socket.clientId] != null) {//从会话中删除
            sessions[this.socket.clientId].teardown();
            delete sessions[this.socket.clientId];
            sessionsCount--;
          }
          logger.raw("[rtmp: " + sessionsCount + " sessions]");

          logger.raw("强制断开客户端" + this.clientid+ "推流请求!");
          ...
          pool.getConnection((err,connection)=>{
            ...
            connection.query(sql.anchor.selectIsBaned,[urlInfo.pathname],(error,result)=>{
              connection.release();
              if(error){
                console.log(error);
                return;
              }
              if(result[0].anch_live_status == 2)//被封禁
              {
                let session, sessionID;
                if (sessions[this.socket.clientId] != null) {//从会话中删除
                  sessions[this.socket.clientId].teardown();
                ...
                logger.raw("强制断开客户端" + this.clientid+ "推流请求!"+"封禁状态!禁止推流");
                ...
                pool.getConnection((err,connection)=>{
                    ...
                    connection.query(sql.anchor.selectUrlByMd5,[urlInfo.pathname],(error,result)=>{
                      connection.release();
                      if(error){
                        console.log(error);
                        return;
                      }
                            publishingName = appName + '/' + result[0].anch_live_url;
                            this.streamId = publishingName;
                            stream = avstreams.get(result);
                            if (stream != null) {
                              stream.reset();
                            } else {
                            ...
                            pool.getConnection(function(err,connection){
                              if (err) throw err;
                              connection.query(sql.anchor.updateStatus,[1,res.anch_live_url],function(error,result){
                                if(error) throw error;
                                console.log("["+res.anch_live_url+"]推流成功");
                              })
                            })
    ...
    setInterval(()=>{//每隔一分钟检测封禁状态
      pool.getConnection((err,connection)=>{
        connection.query(sql.anchor.selectIsBaned,[urlInfo.pathname],(error,result)=>{
          ...
          if(result[0].anch_live_status == 'undefined'){
            return;
          }
          if(result[0].anch_live_status == 2)//被封禁
          {
    ...
    function RTMPServer(opts) {
      ...
      this.server = net.createServer((function(_this) {
        return function(c) {
          ...
          c.on('close', function() {
            logger.info("[rtmp:client=" + sess.clientid + "] 断开连接");
